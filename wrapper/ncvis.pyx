...
cdef class NCVis:
    ...
    def fit(self, float[:, :] X, float a=1., float b=1., float alpha=1., float alpha_Q=1.):
        self.c_ncvis.fit(&X[0, 0], X.shape[0], X.shape[1], a, b, alpha, alpha_Q)
        # cdef cnp.ndarray[cnp.float_t, ndim=2, mode='c']Y = self.c_ncvis.fit(&X[0, 0], X.shape[0], X.shape[1], a, b, alpha, alpha_Q)
